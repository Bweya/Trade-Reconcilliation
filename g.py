...
def trade_graph(d):
    ...
    DealNumber = []
    a_index = 0
    for data in HqTradeData[0]:
        TradeRequestID.append(data)
        DealNumber.append(HqTradeData[2][a_index])#Trans
        a_index+=1
        ...
        for day in range(-d,d+1):
            ...
            for y in DealNumber:
                ...
                for x in usdIndex_get_DealNumbers:
                    ...
                    periodUS = date_133USobj-HqTradeData[8][j]

                    if (date_133USobj.date()).weekday() >= 0 and (HqTradeData[8][j].date()).weekday() < 5 and (date_133USobj.date()).isocalendar()[1] > (HqTradeData[8][j].date()).isocalendar()[1]:

                        periodUS = periodUS-timedelta(days = 2)

                    if month_dict[getmonth] == 1:

                        if HqTradeData[8][j].month == 12:
                        ...
                        if HqTradeData[8][j].month == (month_dict[getmonth] - 1):
                ...
                for x in EURIndex_get_DealNumbers:
                    ...
                    periodEU = date_133EUobj-HqTradeData[8][j]
                    if (date_133EUobj.date()).weekday() >= 0 and (HqTradeData[8][j].date()).weekday() < 5 and (date_133EUobj.date()).isocalendar()[1] > (HqTradeData[8][j].date()).isocalendar()[1]:
