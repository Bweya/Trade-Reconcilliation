...
with open("Report.csv", 'w') as output:
    ...
    trade_writer.writerow(['Index','CO Request ID','Creation Date', 'FX Deal No.','Business Partner','Value Dt', 'Deal Amount', 'Currency', 'Number of Days > 3','Trader' ])
    j = 0
    for y in DealNumber:

        for x in usdIndex_get_DealNumbers:
            if y == FX133TradeData[48][x] and (HqTradeData[8][j]-HqTradeData[15][j]) > timedelta(days = 3):
                #index_in_HQ.append(j+y+FX133TradeData[3][x])
                trade_writer.writerow([ str(j),HqTradeData[0][j],HqTradeData[15][j],str(y),FX133TradeData[41][x],HqTradeData[8][j],"{:,.2f}".format(HqTradeData[9][j]),HqTradeData[7][j],(HqTradeData[8][j]-HqTradeData[15][j]),FX133TradeData[3][x] ])
...
missing_indices = []
for y in range( 0, len(approved_index) ):
